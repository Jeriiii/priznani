	...
	protected function printExceptionEvents(array $events) {
		foreach ($events as $number => $event) {
			...
			if (null !== $exception) {
				...
				$error = sprintf("%s. %s", str_pad((string) ($number + 1), 2, '0', STR_PAD_LEFT), strtr($error, array("\n" => "\n    "))
				);
			...
			if ($this->getParameter('snippets_paths')) {
				...
				foreach ($snippet->getSteps() as $step) {
					$this->writeln(sprintf(
							'{+undefined}%s * %s %s # %s:%d{-undefined}', $indent, $step->getType(), $step->getText(), $this->relativizePathsInString($step->getFile()), $step->getLine()
					));
				}
