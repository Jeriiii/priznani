...
class AdminSpacePresenter extends \Nette\Application\UI\Presenter {
	...
	protected function createComponentAdminMenu($name) {
		...
		if ($user->isInRole("estateAgent") || $user->isInRole("realEstate"))
			...
			if ($user->isAllowed('google_analytics'))
				$navigation["NÁVŠTĚVNOST"] = $this->link("Admin:analytics");
			...
			if ($user->isAllowed('authorizator'))
				$navigation["SUPER NASTAVENÍ"] = $this->link("Admin:authorizator");
		}
	...
	public function beforeRender() {
		...
		$authorizator->setParametrs(
			$allow->galleries, $allow->forms, $allow->accounts, $allow->facebook, $allow->files, $allow->map, $allow->google_analytics, $allow->news
		);
